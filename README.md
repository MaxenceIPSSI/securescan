...
- npm
- Git
- Python 3.10 à 3.13 (requis pour Semgrep — Python 3.14+ non supporté)

---

## Installation de Semgrep

Semgrep est requis pour l'analyse SAST. Il s'installe via pip :

```bash
pip install semgrep
```

Si Python 3.14+ est installé par défaut sur votre machine, installez d'abord Python 3.13 :

```bash
# Windows (via winget)
winget install Python.Python.3.13

# Puis installez Semgrep avec cette version
py -3.13 -m pip install semgrep
```
...
> Sans Semgrep, l'analyse SAST sera ignorée et moins de vulnérabilités seront détectées.
...
Créez un fichier `.env` à partir de `.env.example` et renseignez les variables requises.
